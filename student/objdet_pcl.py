...
def bev_from_pcl(lidar_pcl, configs):
    ...
    intensity_map[np.int_(lidar_top_pcl[:, 0]), np.int_(lidar_top_pcl[:, 1])] = lidar_top_pcl[:, 3] * np.amax(lidar_top_pcl[:, 3]) / (np.amax(lidar_top_pcl[:, 3]) - np.amin(lidar_top_pcl[:, 3]))

    ## step 5 : temporarily visualize the intensity map using OpenCV to make sure that vehicles separate well from the background
    # img_height = intensity_map * 256
    ...
    ##          use the lidar_top_pcl data structure from the previous task to access the pixels of the height_map
